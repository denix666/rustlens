...
pub struct PvcDetailsWindow {
    pub show: bool,
}

impl PvcDetailsWindow {
    pub fn new() -> Self {
        Self {
            show: false,
        }
...
pub fn show_pvc_details_window(
        ctx: &Context,
        pvc_details_window: &mut PvcDetailsWindow,
        details: Arc<Mutex<crate::PvcDetails>>,
        pvcs: Arc<Mutex<Vec<crate::PvcItem>>>,
    ...
    let guard_pvcs = pvcs.lock().unwrap(); // Pvcs with base details already we have
    if guard_details.name.is_none() {
        return;
    }
    let pvc_item = guard_pvcs.iter().find(|item| item.name == guard_details.name.clone().unwrap() && item.namespace == guard_details.namespace.clone());
    if pvc_item.is_none() {
        return;
    }
    let cur_ns = &pvc_item.unwrap().namespace;

    let response = egui::Window::new("Pvc details").min_width(800.0).collapsible(false).resizable(true).open(&mut pvc_details_window.show).show(ctx, |ui| {
        ui.horizontal(|ui| {
        ...
        egui::ScrollArea::vertical().max_height(600.0).show(ui, |ui| {
            egui::Grid::new("pvc_details_grid").striped(true).min_col_width(20.0).show(ui, |ui| {

                ui.label(egui::RichText::new("Pvc name:").color(ROW_NAME_COLOR));
                ui.label(egui::RichText::new(guard_details.name.clone().unwrap()).color(DETAIL_COLOR));
                ui.end_row();

                if let Some(item) = pvc_item {
                    ...
                    ui.label(egui::RichText::new("Storage:").color(ROW_NAME_COLOR));
                    ui.label(egui::RichText::new(&item.size).color(DETAIL_COLOR));
                    ui.end_row();
                }
                ...
                if let Some(labels) = guard_details.labels.clone() {
                    ...
                    egui::Grid::new("pvc_details_labels_grid").striped(true).min_col_width(20.0).show(ui, |ui| {
                ...
                if let Some(annotations) = guard_details.annotations.clone() {
                    ...
                    egui::Grid::new("pvc_details_annotations_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
            ...
            if !guard_details.events.is_empty() {
                egui::Grid::new("pvc_details_events_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
