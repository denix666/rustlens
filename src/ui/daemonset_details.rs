...
pub struct DaemonSetDetailsWindow {
    pub show: bool,
}

impl DaemonSetDetailsWindow {
    pub fn new() -> Self {
        Self {
            show: false,
        }
...
pub fn show_daemonset_details_window(
    ...
    let daemonset_item = guard_daemonsets.iter().find(|item| item.name == guard_details.name.clone().unwrap() && item.namespace == guard_details.namespace.clone());
    if daemonset_item.is_none() {
        return;
    }
    let cur_ns = &daemonset_item.unwrap().namespace;

    let response = egui::Window::new("DaemonSet details").min_width(800.0).collapsible(false).resizable(true).open(&mut daemonset_details_window.show).show(ctx, |ui| {
        ui.horizontal(|ui| {
        ...
        egui::ScrollArea::vertical().max_height(600.0).show(ui, |ui| {
            egui::Grid::new("daemonset_details_grid").striped(true).min_col_width(20.0).show(ui, |ui| {

                ui.label(egui::RichText::new("DaemonSet name:").color(ROW_NAME_COLOR));
                ui.label(egui::RichText::new(guard_details.name.clone().unwrap()).color(DETAIL_COLOR));
                ui.end_row();

                if let Some(item) = daemonset_item {
                ...
                if let Some(labels) = guard_details.labels.clone() {
                    ...
                    egui::Grid::new("daemonset_details_labels_grid").striped(true).min_col_width(20.0).show(ui, |ui| {
                ...
                if let Some(annotations) = guard_details.annotations.clone() {
                    ...
                    egui::Grid::new("daemonset_details_annotations_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
            ...
            if !guard_details.events.is_empty() {
                egui::Grid::new("daemonset_details_events_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
    ...
    if let Some(inner_response) = response {
        if inner_response.response.contains_pointer() && ctx.input(|i| i.key_pressed(Key::Escape)) {
            daemonset_details_window.show = false;
        }
    }
}
