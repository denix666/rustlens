...
pub struct RoleDetailsWindow {
    pub show: bool,
}

impl RoleDetailsWindow {
    pub fn new() -> Self {
        Self {
            show: false,
        }
...
pub fn show_role_details_window(
        ctx: &Context,
        role_details_window: &mut RoleDetailsWindow,
        details: Arc<Mutex<crate::RoleDetails>>,
        roles: Arc<Mutex<Vec<crate::RoleItem>>>,
        yaml_editor_window: Arc<Mutex<YamlEditorWindow>>,
        client: Arc<crate::Client>)
{
    let guard_details = details.lock().unwrap(); // More detailed info
    let guard_roles = roles.lock().unwrap(); // Roles with base details already we have
    if guard_details.name.is_none() {
        return;
    }
    let role_item = guard_roles.iter().find(|item| item.name == guard_details.name.clone().unwrap() && item.namespace == guard_details.namespace.clone());
    if role_item.is_none() {
        return;
    }
    let cur_ns = &role_item.unwrap().namespace;

    egui::Window::new("Service account details").min_width(800.0).collapsible(false).resizable(true).open(&mut role_details_window.show).show(ctx, |ui| {
        ui.horizontal(|ui| {
        ...
        egui::ScrollArea::vertical().max_height(600.0).show(ui, |ui| {
            egui::Grid::new("role_details_grid").striped(true).min_col_width(20.0).show(ui, |ui| {

                ui.label(egui::RichText::new("Role name:").color(ROW_NAME_COLOR));
                ui.label(egui::RichText::new(guard_details.name.clone().unwrap()).color(DETAIL_COLOR));
                ui.end_row();

                if let Some(item) = role_item {
                ...
                if let Some(labels) = guard_details.labels.clone() {
                    ...
                    egui::Grid::new("role_details_labels_grid").striped(true).min_col_width(20.0).show(ui, |ui| {
                ...
                if let Some(annotations) = guard_details.annotations.clone() {
                    ...
                    egui::Grid::new("role_details_annotations_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
            ...
            if !guard_details.events.is_empty() {
                egui::Grid::new("role_details_events_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
