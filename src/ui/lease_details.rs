...
pub struct LeaseDetailsWindow {
    pub show: bool,
}

impl LeaseDetailsWindow {
    pub fn new() -> Self {
        Self {
            show: false,
        }
...
pub fn show_lease_details_window(
        ctx: &Context,
        lease_details_window: &mut LeaseDetailsWindow,
        details: Arc<Mutex<crate::LeaseDetails>>,
        leases: Arc<Mutex<Vec<crate::LeaseItem>>>,
    ...
    let guard_leases = leases.lock().unwrap(); // Leases with base details already we have
    if guard_details.name.is_none() {
        return;
    }
    let lease_item = guard_leases.iter().find(|item| item.name == guard_details.name.clone().unwrap() && item.namespace == guard_details.namespace.clone());
    if lease_item.is_none() {
        return;
    }
    let cur_ns = &lease_item.unwrap().namespace;

    let response = egui::Window::new("Lease details").min_width(800.0).collapsible(false).resizable(true).open(&mut lease_details_window.show).show(ctx, |ui| {
        ui.horizontal(|ui| {
        ...
        egui::ScrollArea::vertical().max_height(600.0).show(ui, |ui| {
            egui::Grid::new("lease_details_grid").striped(true).min_col_width(20.0).show(ui, |ui| {

                ui.label(egui::RichText::new("Lease name:").color(ROW_NAME_COLOR));
                ui.label(egui::RichText::new(guard_details.name.clone().unwrap()).color(DETAIL_COLOR));
                ui.end_row();

                if let Some(item) = lease_item {
                    ...
                    if let Some(holder) = &item.holder {
                        ui.label(egui::RichText::new("Holder:").color(ROW_NAME_COLOR));
                        ui.label(egui::RichText::new(holder).color(DETAIL_COLOR));
                        ui.end_row();
                    }
                }
            });
            ...
            if !guard_details.events.is_empty() {
                egui::Grid::new("lease_details_events_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
