...
pub struct RoleBindingDetailsWindow {
    pub show: bool,
}

impl RoleBindingDetailsWindow {
    pub fn new() -> Self {
        Self {
            show: false,
        }
...
pub fn show_rb_details_window(
        ctx: &Context,
        rb_details_window: &mut RoleBindingDetailsWindow,
        details: Arc<Mutex<crate::RoleBindingDetails>>,
        rbs: Arc<Mutex<Vec<crate::RoleBindingItem>>>,
    ...
    let guard_rbs = rbs.lock().unwrap(); // RoleBindings with base details already we have
    if guard_details.name.is_none() {
        return;
    }
    let rb_item = guard_rbs.iter().find(|item| item.name == guard_details.name.clone().unwrap() && item.namespace == guard_details.namespace.clone());
    if rb_item.is_none() {
        return;
    }
    let cur_ns = &rb_item.unwrap().namespace;

    let response = egui::Window::new("Role binding details").min_width(800.0).collapsible(false).resizable(true).open(&mut rb_details_window.show).show(ctx, |ui| {
        ui.horizontal(|ui| {
                ...
                delete_confirm.request(name.clone(), ns.clone(), move || {
                    tokio::spawn(async move {
                        if let Err(err) = crate::delete_namespaced_component_for::<k8s_openapi::api::rbac::v1::RoleBinding>(
                            ...
                            log::error!("Failed to delete rb: {}", err);
                        }
                    });
                });
            }
        ...
        egui::ScrollArea::vertical().max_height(600.0).show(ui, |ui| {
            egui::Grid::new("rb_details_grid").striped(true).min_col_width(20.0).show(ui, |ui| {

                ui.label(egui::RichText::new("RoleBinding name:").color(ROW_NAME_COLOR));
                ui.label(egui::RichText::new(guard_details.name.clone().unwrap()).color(DETAIL_COLOR));
                ui.end_row();

                if let Some(item) = rb_item {
                ...
                if let Some(labels) = guard_details.labels.clone() {
                    ...
                    egui::Grid::new("rb_details_labels_grid").striped(true).min_col_width(20.0).show(ui, |ui| {
                ...
                if let Some(annotations) = guard_details.annotations.clone() {
                    ...
                    egui::Grid::new("rb_details_annotations_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
            ...
            if !guard_details.events.is_empty() {
                egui::Grid::new("rb_details_events_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
