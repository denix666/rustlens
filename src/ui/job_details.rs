...
pub fn show_job_details_window(
        ctx: &Context,
        ...
        client: Arc<crate::Client>)
{
    ...
    let guard_jobs = jobs.lock().unwrap(); // Jobs with base details already we have
    if guard_details.name.is_none() {
        return;
    }
    let job_item = guard_jobs.iter().find(|item| item.name == guard_details.name.clone().unwrap() && item.namespace == guard_details.namespace.clone());
    if job_item.is_none() {
        return;
    }
    let cur_ns = &job_item.unwrap().namespace;

    egui::Window::new("Job details").min_width(800.0).collapsible(false).resizable(true).open(&mut job_details_window.show).show(ctx, |ui| {
        ui.horizontal(|ui| {
        ...
        egui::ScrollArea::vertical().max_height(600.0).show(ui, |ui| {
            egui::Grid::new("job_details_grid").striped(true).min_col_width(20.0).show(ui, |ui| {

                ui.label(egui::RichText::new("Job name:").color(ROW_NAME_COLOR));
                ...
                if let Some(labels) = guard_details.labels.clone() {
                    ...
                    egui::Grid::new("job_details_labels_grid").striped(true).min_col_width(20.0).show(ui, |ui| {
                ...
                if let Some(annotations) = guard_details.annotations.clone() {
                    ...
                    egui::Grid::new("job_details_annotations_grid").striped(true).min_col_width(20.0).max_col_width(600.0).show(ui, |ui| {
